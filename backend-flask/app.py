...
@app.route("/predictaqi", methods=["GET"])
def predictaqi():
    ...
    print(output)
    result1 = output.to_json(orient="split")
...
def get_pm25_data():
    ...
    if response.status_code == 200:
        ...
        for date in pm25_values:
            pm25_values[date] = str(sum(pm25_values[date]) / len(pm25_values[date]))


        res = []
        for key, val in pm25_values.items():
            newRes = {"date": key, "value": val}
            res.append(newRes)
...
@app.route("/getcityranking", methods=["GET"])
def getcityranking():
    url = "https://api.openaq.org/v1/latest?country=IN&limit=1000"
    response = requests.get(url)
    data = response.json()
    ...
    cities.sort(key=lambda x: x['value'], reverse=True)
    pm25_values = {}
    for measurement in cities:
        date = measurement['city']
        if date in pm25_values:
            pm25_values[date].append(measurement['value'])
        else:
            pm25_values[date] = [measurement['value']]
    ...
    # pm25_values.sort(key=lambda x: x['value'], reverse=True)
    # sorted(pm25_values.items(), key=lambda x: x[1] , reverse=True)
    sorted_value_index = np.argsort(pm25_values.values())
    dictionary_keys = list(pm25_values.keys())
    sort_dictionary = {dictionary_keys[i]:sorted(pm25_values.values())[i] for i in range(len(dictionary_keys))}
